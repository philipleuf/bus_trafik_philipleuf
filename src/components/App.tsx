...
function App() {
  ...
  */
  ...
  React.useEffect(() => {
    const fetcher = async () => {
      ...
      */
      ...
      const [journeyList, stopPointList, siteList] = await Promise.all([fetchData(endPoints.JourneyPattern), fetchData(endPoints.StopPoint), fetchData(endPoints.Site)]);
  ...
  function buildObjectsArray(journeyList: Array<JourneyList>, stopPointList: Array<StopPointList>, siteList: Array<SiteList>) {
  ...
  return (
    <div className="App">
      {top10ListOfBusLines.length === 0 ? (
        ...
        <>
          <Header busLine={busArrMemo} navigateBusLines={(step: number) => navigateBusLines(step)} navigateBusLineIndex={navigateBusLineIndex} />
