...
const findSitePointName = (journeyPointId: string, stopPointList: Array<StopPointList>, siteList: Array<SiteList>) => (stopPointList.find((o: StopPointList) => o.StopPointNumber === journeyPointId))?.StopPointName ||
...
function makeBusStopArray(journeyList: Array<JourneyList>, stopPointList: Array<StopPointList>, siteList: Array<SiteList>) {
    ...
    for (const item of journeyList) {
      ...
      */
      if (busLines[key] && stopPointName) {
        ...
        busLines[key] = { destination: stopPointName, line: item.LineNumber, direction: item.DirectionCode, stopPointList: uniq(busLines[key].stopPointList) };
      } else if (stopPointName) {
        // First hit, finds a matching name, creates new object
        busLines[key] = ({ destination: stopPointName, line: item.LineNumber, direction: item.DirectionCode, stopPointList: [stopPointName] });
      }
    }
    return busLines;
  }
